...
namespace game
{
    internal class Api
    {
        public const string AdminUserId = "fxxmhpje9p";
        private const string AdminToken = "rlcqzcpnpfy";
...
local dir0 = tonumber(@dir0)
local dir5 = tonumber(@dir5)
local now = tonumber(@now)
local game_member = {}
local member_id = nil
...
if dir0 >= 4 and redis.call('hget', 'special_move', game_id..'/'..tostring(member_id)..'/0') then
  return {'already_used'}
end
if dir5 >= 4 and redis.call('hget', 'special_move', game_id..'/'..tostring(member_id)..'/5') then
  return {'already_used'}
end
if redis.call('hsetnx', 'move_hash', game_id..'/'..tostring(turn)..'/'..tostring(member_id), tostring(dir0)..'/'..tostring(dir5)) ~= 1 then
  return {'already_moved'}
end
if dir0 >= 4 then
  redis.call('hset', 'special_move', game_id..'/'..tostring(member_id)..'/0', 1)
end
if dir5 >= 4 then
  redis.call('hset', 'special_move', game_id..'/'..tostring(member_id)..'/5', 1)
end
  ...
  for i = 0, 2 do
    ...
    if game_member[i+1] then
      if game_member[i+1] == '#' then
        ...
        local m = redis.call('hget', 'move_hash', game_id..'/'..tostring(turn)..'/'..tostring(i))
        if m then
          local mm = {}
          for w in string.gmatch(m, '[^/]+') do
            table.insert(mm, w)
          end
          d0 = tonumber(mm[1])
          d5 = tonumber(mm[2])
...
");
        ...
        private static (int, bool) ToDir(string s)
        {
            if (ReNum.IsMatch(s))
            {
                var r = int.Parse(s);
                if (0 <= r && r <= 3) return (r, true);
            }
            else if (s.EndsWith('s'))
            {
                ...
                {
                    var r = int.Parse(t);
                    if (0 <= r && r <= 3) return (r + 4, true);
                }
            }
            else
            {
                var a = s.Split('-');
                if (a.Length == 3 && ReNum.IsMatch(a[0]) && ReNum.IsMatch(a[1]) && ReNum.IsMatch(a[2]))
                {
                    var r0 = int.Parse(a[0]);
                    var r1 = int.Parse(a[1]);
                    var r2 = int.Parse(a[2]);
                    if (0 <= r0 && r0 <= 5 && 0 <= r1 && r1 <= 4 && 0 <= r2 && r2 <= 4) return (25 * r0 + 5 * r1 + r2 + 8, true);
                }
            }
        ...
        private byte[]? RunMove(string rawUrl)
        {
            ...
            var (dir0, ok0) = ToDir(param[2]);
            var (dir5, ok5) = ToDir(param[3]);
            if (!ok0 || !ok5) return null;
            ...
            {
                var now = GetTime();
                var res = (RedisResult[])_runMoveScript1.Evaluate(db, new { userId, gameId, dir0, dir5, now });
                if ((string)res[0] != "ok")
                {
                    return JsonSerializer.SerializeToUtf8Bytes(new Dictionary<string, object>
                    {
