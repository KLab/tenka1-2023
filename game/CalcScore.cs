...
namespace game
{
    internal class CalcScore
    {
        ...
        private static readonly int[,,] FinalMatching = {
            {{3,6,1},{4,0,6},{1,3,7},{1,5,2},{4,0,5},{5,0,6},{4,2,7},{2,3,7}},
            {{5,1,6},{1,7,4},{0,2,6},{2,0,7},{5,3,4},{4,0,3},{5,3,6},{7,2,1}},
            {{7,6,3},{4,1,3},{1,7,5},{2,0,5},{0,2,4},{4,5,6},{0,3,2},{7,6,1}},
            {{4,2,5},{6,2,1},{6,0,7},{2,3,1},{0,7,4},{7,0,5},{3,4,6},{5,3,1}},
            {{4,1,0},{6,5,2},{3,2,4},{1,2,0},{3,0,1},{5,7,3},{4,7,6},{6,7,5}},
            {{4,6,1},{3,0,7},{6,3,0},{0,5,3},{5,2,7},{2,1,4},{4,1,5},{2,7,6}},
            {{5,2,3},{1,7,0},{1,5,0},{1,6,0},{7,4,5},{3,6,2},{7,4,3},{4,6,2}},
        };
...
");
        ...
        public void Start(CancellationToken cts)
        {
            ...
            {
                var s = db.StringGet($"restore_{_startAt}");
                if (s.HasValue)
                {
                    var r = ((string)s).Split(':');
                    userRanking = r[0].Split('/').ToList();
                    rankingTime = long.Parse(r[1]);
                    finalIdx = int.Parse(r[2]);
                    for (var i = 3; i + 3 < r.Length; i += 4)
                    {
                        userData.Add(r[i], new UserData(r[i+1], r[i+2], r[i+3]));
                    }
                }
            }
            ...
            while (!cts.IsCancellationRequested)
            {
...

                {
                    ...
                    {
                        string.Join('/', userRanking),
                        $"{rankingTime}:{finalIdx}"
                    };
                    foreach (var kv in userData)
                    {
                        r.Add(kv.Key);
                        r.Add(kv.Value.RestoreData);
                    }

                    db.StringSet($"restore_{_startAt}", string.Join(':', r));
                }
            }
        }
        ...
        private static void MakeMatches(List<List<string>> matchesList, IEnumerable<string> userIds0)
        {
            ...
            foreach (var matches in matchesList)
            {
                ...
                for (var i = 0; i < n; i += 3)
                {
                    matches.Add(string.Join('/', userIds.Skip(i).Take(3)));
                }
            }
        }
        ...
        private class UserData
        {
            ...
            public UserData(string s0, string s1, string s2)
            {
                _rankList = s0.Split('/').Select(int.Parse).ToList();
                _rankClassList = s1.Split('/').Select(int.Parse).ToList();
                _rankPointList = s2.Split('/').Select(int.Parse).ToList();
            }
            ...
            public string RestoreData
            {
                get
                {
                    var s0 = string.Join('/', _rankList.Select(x => x.ToString()));
                    var s1 = string.Join('/', _rankClassList.Select(x => x.ToString()));
                    var s2 = string.Join('/', _rankPointList.Select(x => x.ToString()));
                    return $"{s0}:{s1}:{s2}";
                }
            }
